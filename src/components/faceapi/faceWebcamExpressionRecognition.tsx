...
import { changeFaceDetector, docElId, getFaceDetectorOptions, isFaceDetectionModelLoaded } from './faceAPIHelper';
...
export default function FaceExpressionRecognitionWebcam() {
  ...
  function onChangeHideBoundingBoxes(e) {
    withBoxes = !docElId(e.target).prop('checked')
  }

  function updateTimeStats(timeInMs) {
    forwardTimes = [timeInMs].concat(forwardTimes).slice(0, 30)
    const avgTimeInMs = forwardTimes.reduce((total, t) => total + t) / forwardTimes.length
    docElId('time').val(`${Math.round(avgTimeInMs)} ms`)
    docElId('fps').val(`${faceapi.utils.round(1000 / avgTimeInMs)}`)
  }

  async function onPlay() {
    const videoEl = docElId('inputVideo')
